...
void *TCP_Analyzer(void *pPara)
{
    ...
    if (NULL == pPara)
    {
        printf("TCP_Analyzer: TCP_Analyzer Failed!\n");
        return NULL;
    }
	...
	if(*pConnfd < 0)
	{
		printf("TCP_Analyzer:Get pConnfd Failed \n" );
		return NULL;
	}
	...
	while(1)
	{
		...
		{
			printf("TCP_Analyzer:remote client close\n" );
			close(*pConnfd);
			free(pConnfd);
			return NULL;
		}
		...
		{
			printf("fd:%d,TCP_Analyzer:%s,%d\n",*pConnfd,Buf,(int)Size);
		}
	}
	close(*pConnfd);
	free(pConnfd);
...
void *TCPServer()
{
	...
	printf("TCPServer:bind socket success\n");
	...
	printf("TCPServer:listen socket success\n");
	...
	while (1)
	{
		...
		switch(select(Listenfd + 1, &fds,NULL, NULL, &timeout ))
		{
			case -1:
	            ...
	            printf("main: create TCP server select fail(%d) %s.\n", errno, strerror(errno));
				return NULL;
				...
				if (FD_ISSET(Listenfd, &fds))
				{
					...
					if (Connfd != -1)
					{
						...
						*pConnectfd = Connfd;
						...
						if(0 != ret)
						{
							printf("TCPServer: TCP_Analyzer build Fail!\n");
